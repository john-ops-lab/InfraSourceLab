...
它的首要目标不是替代现有 Mock / Simulator，而是解决一个更上层的问题：
...
### 3.3 Use Real Protocols Without Reimplementing Them
...
### 3.4 Test the Failure, Not Only the Happy Path
...
### 3.5 Ground Truth Is a Product Feature
...
```text
用户描述
“模拟一家中型企业：2 个机房、300 台服务器、
2 个 vCenter、1200 台 VM、3 个 K8s 集群……”
       ↓
...
Diff
       ↓
用户 Apply 到 Working Copy
       ↓
用户 Save Scenario Revision
       ↓
Compile
       ↓
Start Lab
...
- React/Ant Design/Monaco Web；
- FastAPI Control；
- PostgreSQL；
- 独立 Lab Agent；
- Docker Compose 本地部署；
- Driver Registry / capability API；
- Scenario Catalog、编辑器和 revision 基础模型；
- 复用 DLR 的 UI 和 AI 面板交互结构，但不实现业务 AI；
- health / logs / run shell。
...
- 一个通用 HTTP/OpenAPI driver；
- 至少一个真实服务 driver（建议 PostgreSQL）；
...
- libvirt test driver；
- NetBox demo source；
- 真实 Redis / Kafka / MQTT / SFTP 等 service pack；
- record/replay。

### M5：AI Scenario Assistant

- Prompt → Candidate；
- schema/capability-aware generation；
- attachments/import；
- read-only tools；
- Diff/Apply；
...
第一阶段真正成功，不是“UI 做出来”，而是用户能完成以下闭环：

1. 创建一个带 seed 的 Scenario；

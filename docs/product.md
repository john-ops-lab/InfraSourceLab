...
- 正在开发或学习 CMDB 的个人开发者；
- 正在开发 CMDB 数据采集、导入或接口功能的开发者；
- 需要基础设施配置测试数据的后端、测试或数据治理人员；
- 没有 vCenter、SNMP、云平台、资产系统等真实环境，但需要验证 CMDB 数据采集和导入的人。
...
真实 vCenter、SNMP、Redfish 等协议，只有出现明确且反复发生的 CMDB 数据源测试需求后才单独评估。
...
用户下载 JSON、CSV 或可选 XLSX，用于 CMDB 导入、接口测试或人工检查。

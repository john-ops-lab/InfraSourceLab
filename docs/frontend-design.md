...
内置属性在界面中统一显示为“中文名（英文键名）”，例如“主机名（hostname）”。
这只是展示层翻译：搜索、API、下载文件和持久化数据仍使用英文键名；未登记的自定义
字段原样显示，前端不猜测含义。
...
- 默认限制可见节点数量；
- 支持 CI 类型、关系类型和文字筛选；
- 点击节点后从右侧打开完整 CI 详情：基础信息、全部属性、全部标签分组展示，字段名使用“中文名（英文键名）”；标题与操作区固定，正文过长时在面板内部显示纵向滚动条；
- 支持适配视图、缩放和平移；
- 不编辑拓扑；
- 不引入图数据库；
- 不承诺一次渲染 10,000 个节点。

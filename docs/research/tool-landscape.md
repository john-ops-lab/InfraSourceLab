...
适合未来：需要较复杂请求匹配和录制回放的 HTTP 数据源测试。
...
未来适合测试 CMDB 的 Kubernetes 数据采集，不需要现在接入。
...
未来若要测试 NetBox 数据采集，优先启动真实 NetBox 并通过官方接口写入数据。
